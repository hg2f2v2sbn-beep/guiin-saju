...
(function(root){
  ...
  function coreSections(c){
    ...
    rows.push(section('핵심 진단',
      diagnosisTitle,
      `${n}님의 중심은 ${c?.pillars?.day?.ko||c?.dayMaster?.stem||'일간'}이고, 태어난 달은 ${c?.pillars?.month?.branch||''}월(${season||'계절'})입니다. 같은 일간이라도 계절이 달라지면 힘을 쓰는 방식이 달라지는데, ${n}님은 ${climate}에 가깝습니다.\n\n오행에서는 ${hi[0]} ${hi[1]}%가 가장 두드러지고 ${lo[0]} ${lo[1]}%가 가장 낮습니다. 그래서 ${josa(high.asset,'은/는')} 생각하기 전에 먼저 나오기 쉽고, 반대로 ${josa(low.low,'은/는')} 일부러 구조를 만들어야 안정됩니다.\n\n한마디로 정리하면, ${shape}입니다. 능력의 부족보다 ‘무엇을 자동으로 너무 많이 쓰는가’를 보는 편이 이 명식을 더 정확하게 읽는 방법입니다.`,
      c,[`${hi[0]} ${hi[1]}%`,`${lo[0]} ${lo[1]}%`,stage?`일지 12운성 ${stage}`:null]
    ));

    rows.push(section('구조를 뜯어보면',
      `겉으로 보이는 성격보다 월령과 뿌리가 ${n}님의 실제 반응 속도를 더 잘 설명합니다.`,
      `월주는 ${c?.pillars?.month?.ko||'-'}이고 십성은 ${mg}입니다. ${josa(mg,'은/는')} ${josa(g.core,'과/와')} 연결되며, 사회에서는 특히 ${josa(g.asset,'이/가')} 강점으로 쓰이기 쉽습니다.\n\n일간의 뿌리는 지장간에서 ${r.length}곳 확인됩니다. ${r.length?`이는 힘이 빠져도 내부 기준을 다시 끌어올릴 통로가 ${r.length}곳 있다는 뜻으로 볼 수 있습니다.`:'같은 천간의 뿌리가 직접 확인되지 않는 만큼, 컨디션과 환경이 실력 발휘에 더 직접적인 영향을 줄 수 있습니다.'}\n\n${stage?`일지의 12운성은 ${euro(stage)}, ${stageMeaning(stage)}입니다. `:''}이 요소들을 합치면 ${n}님은 ‘무조건 강하다/약하다’보다, 특정 조건에서 힘이 급격히 살아나거나 급격히 소모되는 타입으로 보는 편이 정확합니다.`,
      c,[`월주 십성 ${mg}`,`통근 ${r.length}곳`,stage?`12운성 ${stage}`:null]
    ));


    const strength=strengthAssessment(c), climateInfo=climateAssessment(c), placement=placementSummary(c), signatures=patternSignatures(c);

    rows.push(section('일간 지지력 · 신강신약 참고',
      `${n}님의 일간 지지력은 현재 ${strength.label}으로 읽힙니다. 중요한 건 강약의 이름보다, 힘이 어디에서 보충되고 어디에서 빠지는지입니다.`,
      `신강·신약은 오행 퍼센트 하나로 정하지 않습니다. 월령의 계절 힘, 지장간 통근, 일간과 같은 오행, 나를 생하는 인성 오행, 그리고 식상·재성·관성이 힘을 얼마나 빼거나 누르는지를 같이 봅니다.\n\n${n}님은 월령 ${strength.monthEl||'-'}의 영향을 받고, 통근은 ${r.length}곳입니다. 이 조합을 내부 해석 지표로 합치면 ${strength.score}/100 정도의 지지력으로 읽힙니다. 이 숫자는 길흉 점수가 아니라 ‘혼자 힘을 얼마나 오래 유지하는가’를 비교하기 위한 값입니다.\n\n${strength.operating}\n\n전문가적으로 중요한 부분은 여기입니다. 같은 ${c?.dayMaster?.stem||''} 일간이어도 이 지지력이 다르면 한 사람은 ‘힘을 빼는 법’이 과제가 되고, 다른 사람은 ‘힘을 붙이는 환경’이 과제가 됩니다.`,
      c,[strength.evidence,`일간 지지력 ${strength.label} ${strength.score}/100`]
    ));

    rows.push(section('조후 · 한난조습',
      `${n}님의 명식은 ${climateInfo.tempLabel}, ${climateInfo.moistureLabel}으로 읽힙니다.`,
      `조후는 “무슨 색을 쓰면 좋다”는 식의 행운 처방이 아니라, 명식이 열·냉기·건조·습윤 중 어느 쪽으로 기울어 있는지를 보는 전통적 관점입니다. 여기서는 계절과 오행 분포를 이용해 생활 리듬에 번역합니다.\n\n현재 구조에서는 ${climateInfo.evidence}가 핵심입니다. ${climateInfo.operating}\n\n이걸 성격으로 단순 번역하면 안 됩니다. 같은 열이 강한 명식도 어떤 사람에게는 실행 속도로, 다른 사람에게는 감정 반응 속도로 나타납니다. 그래서 조후는 반드시 월주 십성·일지·합충과 함께 읽어야 합니다.`,
      c,[climateInfo.evidence]
    ));

    rows.push(section('십성이 어디에 놓였는가',
      `같은 십성도 년주·월주·일지·시주 어디에 있느냐에 따라 밖에서 쓰는 방식과 가까운 사람에게 쓰는 방식이 달라집니다.`,
      `${placement.text}\n\n${placement.year?godPositionMeaning(placement.year,'year'):''}${placement.year?'\n\n':''}${placement.month?godPositionMeaning(placement.month,'month'):''}${placement.month?'\n\n':''}${placement.dayHidden?godPositionMeaning(placement.dayHidden,'dayHidden'):''}${placement.dayHidden?'\n\n':''}${placement.hour?godPositionMeaning(placement.hour,'hour'):''}\n\n${placement.month&&placement.dayHidden&&placement.month!==placement.dayHidden?`특히 사회에서는 ${placement.month}, 가까운 관계에서는 ${placement.dayHidden}이 더 먼저 켜질 수 있어 “밖에서는 잘하는데 왜 가까운 사람 앞에서는 다르지?”라는 차이가 생길 수 있습니다. 이건 모순이 아니라 위치가 다른 십성이 다른 상황에서 작동하는 것입니다.`:'공적 역할과 사적 반응의 차이는 십성 하나보다 월령·일지·합충을 함께 볼 때 더 선명해집니다.'}`,
      c,[placement.text]
    ));

    rows.push(section('이 명식의 핵심 패턴',
      `${n}님을 설명하는 핵심은 성격 단어 하나가 아니라, 반복해서 켜지는 작동 패턴입니다.`,
      signatures.map((x,i)=>`${i+1}. ${x.name} · ${x.text}`).join('\n\n')+
      `\n\n이 네 가지가 서로 연결될 때 “나는 왜 어떤 상황에서는 굉장히 빠르고, 어떤 상황에서는 유난히 지치는가”가 설명됩니다. 장점과 단점이 따로 있는 게 아니라 같은 구조가 조건에 따라 서로 다른 얼굴을 보이는 것입니다.`,
      c,[`일간 지지력 ${strength.label}`,`${climateInfo.tempLabel}·${climateInfo.moistureLabel}`,`월주 ${mg}`]
    ));

    rows.push(section('실력이 터지는 조건',
      `${n}님은 자유가 많을 때보다, 책임과 결정권이 같은 방향을 볼 때 가장 강합니다.`,
      `${josa(high.asset,'이/가')} 장점으로 바뀌는 조건은 분명합니다. 목표가 구체적이고, 내가 어디까지 책임지는지 보이며, 결과를 직접 확인할 수 있을 때입니다. 이때는 ${g.asset}까지 같이 붙으면서 속도와 완성도가 동시에 올라갈 수 있습니다.\n\n반대로 책임은 큰데 권한이 없거나, 계속 사람 감정을 대신 처리해야 하는 환경에서는 같은 장점이 피로로 뒤집힙니다. 잘해서 맡는 일이 계속 늘어나는 구조가 오래가면 ‘왜 결국 내가 다 하지?’라는 감정이 뒤늦게 올라오기 쉽습니다.\n\n칭찬을 하나 정확히 하자면, ${n}님은 아무 데서나 강한 사람이 아니라 구조가 맞는 자리에서 유난히 강한 사람입니다. 그래서 환경 선택이 실력만큼 중요합니다.`,
    ...
    rows.push(section('남들이 오해하기 쉬운 점',
      ...
      `겉으로는 반응이 빠르거나 단호해 보여도, 실제 안쪽에서는 ${mg}의 ${josa(g.core,'과/와')} ${lo[0]} 보완 문제가 같이 돌아갈 수 있습니다. 그래서 어떤 일은 즉시 결정하면서도, 중요한 관계나 책임 문제에서는 예상보다 오래 생각할 수 있습니다.\n\n주변에서는 이 차이를 ‘기분에 따라 바뀐다’고 오해할 수 있지만, 실제로는 결정에 필요한 기준이 분야마다 다른 경우가 많습니다. 익숙한 일은 경험으로 빨리 자르고, 책임이 길게 남는 일은 회복 비용까지 계산하기 때문입니다.\n\n${n}님에게 필요한 건 모든 선택의 속도를 같게 만드는 게 아니라, 상대에게 “지금 결론이 없는 이유”를 한 문장으로 설명하는 것입니다. 설명이 없으면 신중함도 거리두기로 보일 수 있습니다.`,
      c,[`월주 ${mg}`,`${lo[0]} ${lo[1]}%`]
    ));

    rows.push(section('무너질 때 나타나는 순서',
      `${n}님은 갑자기 무너지는 것보다, 잘 버티는 시간이 길어서 한계를 늦게 알아차리는 쪽에 가깝습니다.`,
      `첫 단계에서는 원래 잘하던 ${josa(high.asset,'을/를')} 더 세게 씁니다. 두 번째 단계에서는 ${josa(g.trap,'이/가')} 나타나기 쉬워지고, 세 번째 단계에서는 ${josa(low.low,'을/를')} 챙길 여유가 줄어듭니다.\n\n그래서 밖에서는 여전히 일을 하고 있는데 안에서는 이미 ‘더는 하기 싫다’는 결론이 커질 수 있습니다. 이 상태가 길어지면 작은 수정으로 끝낼 수 있는 일을 한 번에 끊거나 크게 바꾸고 싶어질 수 있습니다.\n\n회복 신호는 의욕보다 먼저 봐야 합니다. 수면·식사·말수·지출·연락 빈도 중 평소와 달라지는 것이 두 가지 이상 생기면, 그때는 능력 문제가 아니라 과사용 신호로 보고 일정을 줄이는 편이 좋습니다.`,
      c,[`${hi[0]} 과사용`,`${mg} 함정`,`${lo[0]} 보완`]
    ));


    rows.push(section('생활 장면으로 번역하면',
      `명식은 추상적인 성격표보다 반복되는 생활 장면으로 번역할 때 훨씬 정확해집니다.`,
      `① 일이 몰릴 때 · ${strength.label}인 ${n}님은 ${strength.score>=58?'처음에는 오히려 집중력이 올라가지만, 한계를 넘으면 “내가 다 하는 구조”가 굳어지는지 봐야 합니다.':'처음부터 버티는 양을 늘리기보다 누구에게 무엇을 요청할지 정했을 때 결과가 더 안정될 수 있습니다.'}\n\n② 가까운 사람이 서운하게 했을 때 · ${relationList(c).some(x=>['충','형','해'].includes(x?.type))?'바로 끝내기보다 안에서 여러 번 판단한 뒤 한 번에 수정 폭을 크게 잡을 수 있습니다.':'관계의 사건 하나보다 반복 패턴과 역할 균형을 더 오래 보는 편일 수 있습니다.'}\n\n③ 큰돈·큰결정을 앞둘 때 · ${mg}의 ${josa(g.core,'이/가')} 켜져, ${josa(g.asset,'이/가')} 장점이 되지만 ${josa(g.trap,'이/가')} 함께 나타나는지 확인해야 합니다.\n\n이 세 장면에서 같은 반응이 반복된다면 그건 우연한 기분보다 명식의 자동 운용 방식에 가깝습니다. 반대로 실제 삶과 다르다면 그 부분은 해석보다 현실 경험을 우선해야 합니다.`,
      c,[`일간 지지력 ${strength.label}`,`월주 ${mg}`,relationText(c)]
    ));

    rows.push(section('결정 방식',
      `${n}님은 우유부단해서 늦는 사람이 아니라, 납득 기준이 채워질 때까지 보류하는 결정이 있는 사람입니다.`,
      `${mg}의 ${josa(g.core,'과/와')} ${hi[0]} 우세가 함께 작동하면, 익숙한 분야에서는 판단이 매우 빨라질 수 있습니다. 반대로 책임 범위가 모호하거나 정보가 부족한 문제에서는 결론을 미루며 머릿속 시뮬레이션이 길어질 수 있습니다.\n\n그래서 ‘결정을 빨리 해라’는 조언은 잘 맞지 않습니다. 대신 큰 선택을 할 때 ① 내가 책임질 범위 ② 6개월 뒤 유지 가능성 ③ 실패했을 때 회복 비용, 이 세 가지를 먼저 적는 편이 훨씬 잘 맞습니다.\n\n이 세 항목이 선명하면 ${n}님은 결정을 오래 끌기보다 실행으로 옮기는 속도가 빨라지는 편입니다.`,
      c,[`월주 ${mg}`,`${hi[0]} ${hi[1]}%`]
    ));

    rows.push(section('일과 돈',
      `돈은 많이 버는 운보다, ${n}님이 어떤 방식으로 벌 때 덜 새는지를 보는 편이 더 중요합니다.`,
      `일에서는 ${josa(g.asset,'이/가')} 수입과 연결되기 쉽습니다. ${hi[0]}의 ${josa(high.asset,'을/를')} 결과물·기술·고객·성과처럼 반복 가능한 형태로 바꾸면 장점이 오래갑니다.\n\n반대로 바쁠수록 돈을 ‘시간을 사는 비용’이나 ‘관계를 유지하는 비용’으로 쓰기 시작하면 지출이 눈에 띄게 늘 수 있습니다. 특히 ${lo[0]} 기능이 낮을 때는 ${josa(low.low,'이/가')} 약해져, 거절해야 할 비용이나 끊어야 할 고정비가 오래 남을 수 있습니다.\n\n운용법은 단순합니다. 수입이 들어오면 고정비·저축·자유비용을 먼저 나누고, 다른 사람을 위해 쓰는 비용은 별도 항목으로 보이게 하세요. 숫자가 보이면 감정으로 대신 부담하는 돈이 줄어듭니다.`,
    ...
    if(lk||yg){
      ...
      rows.push(section('지금의 시간축',
        `지금은 운의 이름보다, 대운과 세운이 원국의 어느 자리를 실제로 건드리는지가 더 중요합니다.`,
        `${lk?`현재 대운은 ${lk.ko}(${lk.god})입니다. ${josa(GOD[lk.god]?.core||'특정 역할','이/가')} 장기 과제로 올라옵니다.`:''}${yg?` ${tx.year}년의 천간 십성은 ${yg}으로 읽힙니다.`:''}\n\n대운 교차 · ${tx.luckRelations.text}\n\n세운 교차 · ${tx.yearRelations.text}\n\n합이 잡히면 그 자리를 쉽게 지나치지 못하고 조정·연결하려는 힘이 커질 수 있고, 충·형·해·파가 잡히면 그 영역의 기존 방식을 수정해야 한다는 압박을 더 크게 느낄 수 있습니다. 이것을 사건 예언으로 읽지는 않습니다.\n\n따라서 지금의 질문은 “좋은 해인가, 나쁜 해인가”가 아니라 “원국의 어느 기능이 지금 반복해서 호출되고 있는가”입니다. 같은 문제가 일·돈·관계에서 모양만 바꿔 반복된다면 그게 현재 운에서 먼저 손볼 주제입니다.`,
        c,[lk?`현재 대운 ${lk.ko} ${lk.god}`:null,yg?`${tx.year} 세운 ${yg}`:null,tx.luckRelations.text,tx.yearRelations.text]
      ));
    }

    rows.push(section('이 사주의 핵심 과제',
      `${n}님의 과제는 더 강해지는 것이 아니라, 강한 힘을 어디까지 쓸지 스스로 정하는 것입니다.`,
      `이 명식에서 반복해서 보이는 핵심은 ${hi[0]}의 ${josa(high.asset,'과/와')} ${lo[0]}의 보완 문제입니다. 잘할 수 있다는 이유로 계속 맡으면 강점은 결국 소모가 되고, 반대로 선을 너무 일찍 닫으면 원래 가진 실행력과 집중력이 충분히 쓰이지 못합니다.\n\n그래서 평생 운용 원칙은 하나로 압축할 수 있습니다. “할 수 있는가?”를 묻기 전에 “이게 내 몫인가, 그리고 계속 감당할 수 있는가?”를 먼저 묻는 것.\n\n이 질문이 선명해질수록 ${n}님의 장점은 덜 소모되고 더 오래 갑니다.`,
  ...
  function compatBuild(a,b,score){
    ...
    sections.push({id:'difference',category:'summary',title:'같은 일간이어도 전혀 다르게 반응할 수 있는 이유',body:
      `${sameStem?`두 사람은 같은 ${A.stem} 일간입니다. 하지만 일간을 떠받치는 힘은 ${nA}님이 ${A.strength.label}(${A.strength.score}/100), ${nB}님이 ${B.strength.label}(${B.strength.score}/100)으로 서로 다르게 잡힙니다.`:`두 사람의 일간과 지지력 구조가 다릅니다. ${nA}님은 ${A.strength.label}(${A.strength.score}/100), ${nB}님은 ${B.strength.label}(${B.strength.score}/100)입니다.`}\n\n${nA}님은 ${A.climate.tempLabel}·${A.climate.moistureLabel}, ${nB}님은 ${B.climate.tempLabel}·${B.climate.moistureLabel}이라 같은 사건도 체감 속도가 다를 수 있습니다. 한쪽은 이미 결론이 났는데 다른 쪽은 아직 상황을 흡수하는 중일 수 있다는 뜻입니다.\n\n따라서 이 관계에서 “나랑 같은 일간인데 왜 저렇게 다르지?”라는 질문은 맞지 않습니다. 같은 불이어도 계절·뿌리·주변 오행이 다르면 한쪽은 힘을 빼는 것이 과제이고, 다른 쪽은 힘을 붙이는 환경이 과제가 될 수 있습니다.`,
      evidence:`일간 지지력 ${nA} ${A.strength.label} ${A.strength.score}/100 ↔ ${nB} ${B.strength.label} ${B.strength.score}/100 · 조후 ${A.climate.tempLabel}/${B.climate.tempLabel}`});
    ...
    sections.push({id:'contact',category:'love',title:'연락과 거리감에서 생기는 오해',body:
      `${nA}님은 ${A.mg}의 ‘${GOD[A.mg]?.core||A.mg}’, ${nB}님은 ${B.mg}의 ‘${GOD[B.mg]?.core||B.mg}’가 사회적 반응에 먼저 묻어날 수 있습니다. 여기에 일간 지지력 차이까지 겹치면 한 사람에게는 “조금 생각할 시간”이 필요한데 다른 사람에게는 “왜 바로 답이 없지?”로 느껴질 수 있습니다.\n\n연락 횟수 자체를 애정의 크기로 환산하면 이 관계는 쉽게 피곤해집니다. 더 중요한 건 연락이 늦어질 때 설명하는 방식, 다시 연결되는 시간, 혼자 있는 시간의 기준입니다.\n\n실전에서는 ‘답이 늦어질 수 있는 상황 / 꼭 바로 답해야 하는 상황 / 하루 안에 공유해야 하는 일’을 세 단계로 나누는 편이 감정 소모를 크게 줄입니다.`,
      evidence:`월주 십성 ${A.mg} ↔ ${B.mg} · 일간 지지력 ${A.strength.score} ↔ ${B.strength.score}`});

    sections.push({id:'repair',category:'love',title:'싸운 뒤 누가 먼저 회복되는가',body:
      `갈등 뒤 회복 속도는 사랑의 크기보다 각자의 에너지 복구 방식과 더 관련이 있습니다. ${nA}님은 ${A.strength.operating} ${nB}님은 ${B.strength.operating}\n\n${neg.length?`특히 ${hitSummary(neg.slice(0,3))} 같은 긴장 신호가 있을 때는, 문제를 끝냈다고 생각하는 시점이 서로 다를 수 있습니다.`:'직접적인 강한 충돌 신호가 적다면 말투와 역할 기대가 회복 속도를 더 크게 좌우할 수 있습니다.'}\n\n한쪽이 정리되었다고 해서 다른 쪽도 끝난 건 아닙니다. 화해 규칙을 “사과했으니 끝”이 아니라 ① 사실 확인 ② 감정 확인 ③ 다음 행동 합의로 나누면 같은 싸움을 반복하는 빈도가 줄어듭니다.`,
      evidence:`긴장 교차 ${hitSummary(neg.slice(0,3))} · 지지력 ${A.strength.label}/${B.strength.label}`});

    sections.push({id:'boundary',category:'marriage',title:'가족·친구·돌봄이 끼어들 때',body:
      `두 사람 사이만 좋으면 끝나는 관계가 아닐 수 있습니다. 반복되는 가족 일정, 친구 관계, 반려동물, 아픈 사람 돌봄처럼 제3의 책임이 들어오면 각자 원래 갖고 있던 책임 습관이 더 선명해집니다.\n\n${nA}님은 ${A.mg}의 ${GOD[A.mg]?.trap||'과사용'}, ${nB}님은 ${B.mg}의 ${josa(GOD[B.mg]?.trap||'과사용','이/가')} 과해질 때 관계 밖의 일을 관계 안으로 가져올 수 있습니다.\n\n그래서 제3자의 문제를 도와줄 때는 ‘누가 결정하는가 / 비용은 누가 부담하는가 / 언제까지 돕는가’를 먼저 정해야 합니다. 이 경계가 없으면 좋은 의도가 한쪽의 고정 책임으로 바뀔 수 있습니다.`,
      evidence:`월주 십성 ${A.mg}/${B.mg} · 약한 오행 ${A.lo[0]}/${B.lo[0]}`});
    ...
    return {meta:{version:'deep-v3',aName:nA,bName:nB,score:Number.isFinite(score)?score:null,crossTenGod:{aSeesB:aSees,bSeesA:bSees},positiveSignals:pos,negativeSignals:neg,allSignals:hits},sections};
  }
