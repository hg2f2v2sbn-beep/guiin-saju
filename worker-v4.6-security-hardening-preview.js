...
const WORKER_VERSION="4.6-security-hardening-preview";
...
export default{
  async fetch(request,env){
    ...
    if(request.method==="GET"&&(url.pathname==="/"||url.pathname==="/health")){
      return json({ok:true,service:"guiin-saju-api",version:WORKER_VERSION,answerSchemaVersion:ANSWER_SCHEMA_VERSION,chatEnabled:env.AI_CHAT_ENABLED!=="false",databaseReady:dbReady(env),serverAccountingReady:false,paymentsEnabled:false,persistencePreview:true,accountingPreview:true,paymentFoundationPreview:true,paymentRecoveryPreview:true,securityHardeningPreview:true},200,o);
    }
    ...
    if(url.pathname.startsWith("/api/admin/")){
      await recordSecurityEvent(env,{eventType:"admin_route_blocked",severity:"critical",requestId:reqId,route:url.pathname,request,subject:s});
      return json({ok:false,error:"admin_access_not_configured",requestId:reqId},403,o);
    }
