...
const WORKER_VERSION="5.0-account-link-preview";
...
async function guestFromExplicitHeader(env,request){
  const t=(request.headers.get("X-Guiin-Guest")||"").trim();
  if(!/^gst_[A-Za-z0-9._:-]{32,}$/.test(t))return null;
  return guestSession(env,request);
}

async function activeGuestReservations(env,guestId){
  const w=await env.DB.prepare(`SELECT COUNT(*) AS c FROM wallet_reservations WHERE subject_type='guest' AND subject_id=? AND state='RESERVED'`).bind(guestId).first();
  const q=await env.DB.prepare(`SELECT COUNT(*) AS c FROM quota_reservations WHERE subject_type='guest' AND subject_id=? AND state='RESERVED'`).bind(guestId).first();
  return {wallet:Number(w?.c||0),quota:Number(q?.c||0)};
}

async function linkGuestToUser(env,{guestId,userId,idempotencyKey,requestId}){
  const old=await env.DB.prepare(`SELECT id,user_id,state,summary_json FROM account_link_events WHERE idempotency_key=? LIMIT 1`).bind(idempotencyKey).first();
  if(old){
    if(String(old.user_id)!==String(userId))throw new Error("link_idempotency_conflict");
    if(old.state==="COMPLETED")return {replayed:true,summary:JSON.parse(old.summary_json||"{}")};
    if(old.state==="PROCESSING")throw new Error("link_in_progress");
  }

  const guest=await env.DB.prepare(`SELECT id,converted_user_id,revoked_at FROM guest_sessions WHERE id=? LIMIT 1`).bind(guestId).first();
  if(!guest)throw new Error("guest_not_found");
  if(guest.converted_user_id&&String(guest.converted_user_id)!==String(userId))throw new Error("guest_already_linked");
  if(guest.converted_user_id&&String(guest.converted_user_id)===String(userId))return {replayed:true,alreadyLinked:true};

  const reservations=await activeGuestReservations(env,guestId);
  if(reservations.wallet>0||reservations.quota>0)throw new Error("active_reservation_exists");

  const now=new Date().toISOString(),eventId=uid("link");
  await env.DB.prepare(`INSERT INTO account_link_events(id,guest_session_id,user_id,state,idempotency_key,created_at)
    VALUES(?,?,?,?,?,?)`).bind(eventId,guestId,userId,"PROCESSING",idempotencyKey,now).run();

  const uw=await env.DB.prepare(`SELECT balance,reserved_balance FROM wallet_accounts WHERE subject_type='user' AND subject_id=? LIMIT 1`).bind(userId).first();
  const gw=await env.DB.prepare(`SELECT balance,reserved_balance FROM wallet_accounts WHERE subject_type='guest' AND subject_id=? LIMIT 1`).bind(guestId).first();
  const mergedBalance=Number(uw?.balance||0)+Number(gw?.balance||0);

  const uq=await env.DB.prepare(`SELECT used_count,reserved_count,limit_count FROM usage_quotas WHERE subject_type='user' AND subject_id=? AND quota_key='ai_chat_free' AND period_key='lifetime' LIMIT 1`).bind(userId).first();
  const gq=await env.DB.prepare(`SELECT used_count,reserved_count,limit_count FROM usage_quotas WHERE subject_type='guest' AND subject_id=? AND quota_key='ai_chat_free' AND period_key='lifetime' LIMIT 1`).bind(guestId).first();
  const limit=Math.max(Number(uq?.limit_count||3),Number(gq?.limit_count||3),3);
  const mergedUsed=Math.min(limit,Number(uq?.used_count||0)+Number(gq?.used_count||0));
  ...
  const statements=[
    ...
    env.DB.prepare(`DELETE FROM wallet_accounts WHERE subject_type='guest' AND subject_id=?`).bind(guestId),
    ...
    env.DB.prepare(`DELETE FROM usage_quotas WHERE subject_type='guest' AND subject_id=?`).bind(guestId),

    env.DB.prepare(`UPDATE profiles SET user_id=?,guest_session_id=NULL,updated_at=? WHERE guest_session_id=?`).bind(userId,now,guestId),
    env.DB.prepare(`UPDATE chart_snapshots SET user_id=?,guest_session_id=NULL WHERE guest_session_id=?`).bind(userId,guestId),
    env.DB.prepare(`UPDATE conversations SET user_id=?,guest_session_id=NULL,updated_at=? WHERE guest_session_id=?`).bind(userId,now,guestId),
    env.DB.prepare(`UPDATE reports SET user_id=?,guest_session_id=NULL,updated_at=? WHERE guest_session_id=?`).bind(userId,now,guestId),
    env.DB.prepare(`UPDATE orders SET user_id=?,guest_session_id=NULL,updated_at=? WHERE guest_session_id=?`).bind(userId,now,guestId),
    env.DB.prepare(`UPDATE entitlements SET user_id=?,guest_session_id=NULL WHERE guest_session_id=?`).bind(userId,guestId),
    env.DB.prepare(`UPDATE consents SET user_id=?,guest_session_id=NULL WHERE guest_session_id=?`).bind(userId,guestId),
    env.DB.prepare(`UPDATE ai_requests SET user_id=?,guest_session_id=NULL,updated_at=? WHERE guest_session_id=?`).bind(userId,now,guestId),
    env.DB.prepare(`UPDATE wallet_ledger SET subject_type='user',subject_id=? WHERE subject_type='guest' AND subject_id=?`).bind(userId,guestId),

    env.DB.prepare(`UPDATE guest_sessions SET converted_user_id=?,revoked_at=?,revoke_reason='converted_to_user' WHERE id=?`).bind(userId,now,guestId)
  ];
  await env.DB.batch(statements);

  const summary={walletBalance:mergedBalance,freeUsed:mergedUsed,freeLimit:limit};
  await env.DB.prepare(`UPDATE account_link_events SET state='COMPLETED',summary_json=?,completed_at=? WHERE id=?`)
    .bind(JSON.stringify(summary),now,eventId).run();
...
export default{
  async fetch(request,env){
    ...
    if(request.method==="POST"&&url.pathname==="/api/auth/link-guest"){
      if(!dbReady(env))return json({ok:false,error:"database_unavailable",requestId:reqId},503,o);
      const u=await userSession(env,request);
      if(!u)return json({ok:false,error:"user_session_required",requestId:reqId},401,o);
      const g=await guestFromExplicitHeader(env,request);
      if(!g)return json({ok:false,error:"guest_session_required",requestId:reqId},401,o);
      ...
      try{
        const linked=await linkGuestToUser(env,{guestId:g.id,userId:u.user_id,idempotencyKey:idem,requestId:reqId});
        return json({ok:true,requestId:reqId,linked:true,...linked},200,o);
      }catch(e){
        const code=String(e?.message||"link_failed");
        const status=code==="active_reservation_exists"||code==="link_in_progress"?409:
          code==="guest_already_linked"||code==="link_idempotency_conflict"?409:500;
