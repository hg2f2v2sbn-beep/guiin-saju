...
const WORKER_VERSION="4.2-ai-transaction-preview";
...
export default{
  async fetch(request,env){
    ...
    if(request.method==="GET"&&(url.pathname==="/"||url.pathname==="/health")){
      return json({ok:true,service:"guiin-saju-api",version:WORKER_VERSION,answerSchemaVersion:ANSWER_SCHEMA_VERSION,chatEnabled:env.AI_CHAT_ENABLED!=="false",databaseReady:dbReady(env),serverAccountingReady:false,paymentsEnabled:false,persistencePreview:true},200,o);
    }
    ...
    if(request.method==="POST"&&url.pathname==="/api/chat"){
      ...
      try{
        ...
        return json({
          ...
          durableResultStored:stored.state==="STORED",
          chargeSafe:true,serverCharged:false,serverAccountingReady:false
        },200,o);
      }catch(e){
