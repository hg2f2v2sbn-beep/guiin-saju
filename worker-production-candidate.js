...
const WORKER_VERSION="6.1-production-candidate";
const FINAL_PAYMENTS_FAIL_CLOSED=true;
...
function corsHeaders(o){return {
  ...
  "Access-Control-Allow-Headers":"Content-Type, Accept, Authorization, X-Guiin-Guest, X-Session-ID, X-Idempotency-Key",
...
async function guestSession(env,req){
  if(!dbReady(env))return null;
  const t=(req.headers.get("X-Guiin-Guest")||req.headers.get("X-Session-ID")||"").trim();
...
function cleanHistory(h){if(!Array.isArray(h))return [];return h.filter(x=>x&&(x.role==="user"||x.role==="assistant")&&typeof x.content==="string").slice(-6).map(x=>({role:x.role,content:x.content.trim().slice(0,1200)})).filter(x=>x.content)}
...
function cleanAnswer(a){
  ...
  s=s.replace(/(?:사용자님|고객님|내담자님|회원님|당신)[,，]?\s*/g,"");
  s=s.replace(/\n{3,}/g,"\n\n").trim();
  return s;
}
...
async function callOpenAI(env,messages,maxTokens,id){
  ...
  if(!r.ok){throw Object.assign(new Error("ai_request_failed"),{status:r.status})}
...
async function generateAnswer(env,facts,history,q,id){
  const first=await callOpenAI(env,[{role:"system",content:SYSTEM_PROMPT},{role:"system",content:`현재 서버 기준 날짜는 ${koreaDate()}입니다.`},{role:"system",content:chartContext(facts)},...history,{role:"user",content:q}],950,id);
  const cleaned=cleanAnswer(first.answer);
  const issues=qualityIssues(cleaned);
  if(!issues.length)return {answer:cleaned,model:first.model,quality:{repaired:false,issues:[]}};
  const repaired=await callOpenAI(env,[{role:"system",content:SYSTEM_PROMPT},{role:"system",content:chartContext(facts)},{role:"user",content:`아래 답변의 확정적·공포성 표현만 안전하고 현실적인 가능성 표현으로 고치세요. 새로운 명리 사실은 추가하지 마세요.\n질문:${q}\n답변:${cleaned.slice(0,6000)}\n최종 답변만 출력하세요.`}],950,id+"_repair");
  const finalAnswer=cleanAnswer(repaired.answer);
  return {answer:finalAnswer,model:repaired.model,quality:{repaired:true,issues,remaining:qualityIssues(finalAnswer)}};
}
...
function routeClass(path){
  ...
  if(path==="/api/me"||path.startsWith("/api/me/"))return"private_read";
...
async function recordSecurityEvent(env,{eventType,severity="warning",subject=null,requestId=null,route="",request,metadata={}}){
...
function runtimeGate(states,path,method){
  ...
  if(path.startsWith("/api/orders")&&write){
    if(paymentsDisabled)return{allowed:false,status:503,error:"payments_disabled"};
  }
  return{allowed:true};
}

async function recordIncident(env,{type,severity="warning",state="OPEN",requestId=null,subject=null,orderId=null,aiRequestId=null,details={}}){
  if(!dbReady(env))return;
  try{
...
async function createLoginState(env,provider,guestSessionId,redirectPath){
  ...
  const requested=String(redirectPath||"/");
  const path=requested==="/demo.html"?"/demo.html":"/";
...
function safeFrontendOrigin(env){
  const raw=String(env.KAKAO_FRONTEND_ORIGIN||"https://gwiinsaju.com").trim();
  try{
    const u=new URL(raw);
    const origin=u.origin;
    return ALLOWED_ORIGINS.has(origin)?origin:"https://gwiinsaju.com";
  }catch(_){
    return "https://gwiinsaju.com";
  }
}

function kakaoRedirectUri(env){
  const v=String(env.KAKAO_REDIRECT_URI||"").trim();
  if(!v)throw new Error("kakao_redirect_uri_missing");
  let u;
  try{u=new URL(v)}catch(_){throw new Error("kakao_redirect_uri_invalid")}
  if(u.protocol!=="https:")throw new Error("kakao_redirect_uri_invalid");
  return u.toString();
}


function kakaoAuthorizeUrl(env,state){
  const clientId=String(env.KAKAO_REST_API_KEY||"").trim();
  if(!clientId)throw new Error("kakao_rest_api_key_missing");
  const u=new URL("https://kauth.kakao.com/oauth/authorize");
  u.searchParams.set("client_id",clientId);
  u.searchParams.set("redirect_uri",kakaoRedirectUri(env));
...
async function exchangeKakaoCode(env,code){
  ...
  form.set("redirect_uri",kakaoRedirectUri(env));
  ...
  let d=null;try{d=await r.json()}catch(_){}
  if(!r.ok||!d?.access_token)throw new Error("kakao_token_exchange_failed");
  return d;
}
...
async function fetchKakaoUser(accessToken){
  ...
  let d=null;try{d=await r.json()}catch(_){}
  if(!r.ok||d?.id==null)throw new Error("kakao_userinfo_failed");
  return d;
}

async function consumeLoginState(env,provider,rawState){
  const stateHash=await sha256(String(rawState||""));
  const row=await env.DB.prepare(`SELECT id,provider,guest_session_id,redirect_path,expires_at,used_at
    FROM auth_login_states
    WHERE state_hash=? AND provider=? LIMIT 1`).bind(stateHash,provider).first();
  if(!row)throw new Error("login_state_not_found");
  if(row.used_at)throw new Error("login_state_already_used");
  const exp=Date.parse(row.expires_at||"");
  if(!Number.isFinite(exp)||exp<=Date.now())throw new Error("login_state_expired");

  const now=new Date().toISOString();
  const used=await env.DB.prepare(`UPDATE auth_login_states SET used_at=?
    WHERE id=? AND used_at IS NULL AND expires_at>?`).bind(now,row.id,now).run();
  if(Number(used?.meta?.changes||0)!==1)throw new Error("login_state_already_used");
  return row;
}

async function getOrCreateKakaoUser(env,kakao){
  const providerUserId=String(kakao.id);
  const account=kakao.kakao_account||{};
  const profile=account.profile||{};
  const displayName=text(profile.nickname||kakao.properties?.nickname,80)||null;
  const emailRaw=typeof account.email==="string"?account.email.trim().toLowerCase():"";
  const emailVerified=account.is_email_verified===true&&account.is_email_valid!==false;
  const emailNormalized=emailVerified&&emailRaw?emailRaw:null;

  const identity=await env.DB.prepare(`SELECT a.user_id,u.status
    FROM auth_identities a
    JOIN users u ON u.id=a.user_id
    WHERE a.provider='kakao' AND a.provider_user_id=? LIMIT 1`).bind(providerUserId).first();

  const now=new Date().toISOString();
  if(identity){
    if(identity.status!=="active")throw new Error("user_not_active");
    await env.DB.batch([
      env.DB.prepare(`UPDATE auth_identities SET email_normalized=?,email_verified=?,updated_at=?
        WHERE provider='kakao' AND provider_user_id=?`).bind(emailNormalized,emailVerified?1:0,now,providerUserId),
      env.DB.prepare(`UPDATE users SET display_name=COALESCE(?,display_name),updated_at=? WHERE id=?`)
        .bind(displayName,now,identity.user_id)
    ]);
    return {userId:identity.user_id,newUser:false,displayName};
  }

  const userId=uid("user"),identityId=uid("ident");
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO users(id,status,display_name,created_at,updated_at)
      VALUES(?,?,?,?,?)`).bind(userId,"active",displayName,now,now),
    env.DB.prepare(`INSERT INTO auth_identities(
      id,user_id,provider,provider_user_id,email_normalized,email_verified,created_at,updated_at
    ) VALUES(?,?,?,?,?,?,?,?)`).bind(
      identityId,userId,"kakao",providerUserId,emailNormalized,emailVerified?1:0,now,now
    ),
    env.DB.prepare(`INSERT OR IGNORE INTO usage_quotas(
      id,subject_type,subject_id,quota_key,period_key,used_count,reserved_count,limit_count,updated_at
    ) VALUES(?,?,?,?,?,?,?,?,?)`).bind(
      uid("quota"),"user",userId,"ai_chat_free","lifetime",0,0,3,now
    ),
    env.DB.prepare(`INSERT OR IGNORE INTO wallet_accounts(
      subject_type,subject_id,balance,reserved_balance,version,updated_at
    ) VALUES(?,?,?,?,?,?)`).bind("user",userId,0,0,0,now)
  ]);
  return {userId,newUser:true,displayName};
}

async function createUserSession(env,userId,request){
  const raw=randomToken("usr_"),id=uid("usess"),now=new Date();
  const idleSeconds=30*24*60*60;
  const expires=new Date(now.getTime()+idleSeconds*1000);
  const absolute=new Date(now.getTime()+180*24*60*60*1000);
  const ipHash=await hashSecurityValue(env,clientIp(request));
  const uaHash=await hashSecurityValue(env,request.headers.get("User-Agent")||"");

  await env.DB.prepare(`INSERT INTO user_sessions(
    id,user_id,token_hash,token_version,created_at,last_seen_at,expires_at,
    absolute_expires_at,idle_timeout_seconds,ip_hash,user_agent_hash
  ) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(
    id,userId,await sha256(raw),1,now.toISOString(),now.toISOString(),expires.toISOString(),
    absolute.toISOString(),idleSeconds,ipHash,uaHash
  ).run();

  return {sessionToken:raw,sessionId:id,expiresAt:expires.toISOString(),absoluteExpiresAt:absolute.toISOString()};
}

async function ensureAuthHandoffTable(env){
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS auth_login_handoffs(
    id TEXT PRIMARY KEY,
...
async function createAuthHandoff(env,userId){
  await ensureAuthHandoffTable(env);
  const raw=randomToken("ah_"),id=uid("ah"),now=new Date(),exp=new Date(now.getTime()+2*60*1000);
  await env.DB.prepare(`DELETE FROM auth_login_handoffs WHERE expires_at<? OR used_at IS NOT NULL`)
    .bind(now.toISOString()).run();
  await env.DB.prepare(`INSERT INTO auth_login_handoffs(
    id,code_hash,session_token,user_id,expires_at,created_at
  ) VALUES(?,?,?,?,?,?)`).bind(
    id,await sha256(raw),"issued_on_exchange",userId,exp.toISOString(),now.toISOString()
  ).run();
  return {code:raw,expiresAt:exp.toISOString()};
}

async function consumeAuthHandoff(env,rawCode){
  await ensureAuthHandoffTable(env);
  const code=String(rawCode||"").trim();
  if(!/^ah_[A-Za-z0-9._:-]{32,}$/.test(code))throw new Error("auth_handoff_invalid");

  const hash=await sha256(code);
  const row=await env.DB.prepare(`SELECT id,session_token,user_id,expires_at,used_at
    FROM auth_login_handoffs WHERE code_hash=? LIMIT 1`).bind(hash).first();

  if(!row)throw new Error("auth_handoff_not_found");
  if(row.used_at)throw new Error("auth_handoff_already_used");
  if(!row.expires_at||Date.parse(row.expires_at)<=Date.now())throw new Error("auth_handoff_expired");

  const now=new Date().toISOString();
  const used=await env.DB.prepare(`UPDATE auth_login_handoffs SET used_at=?
    WHERE id=? AND used_at IS NULL`).bind(now,row.id).run();
  if(Number(used?.meta?.changes||0)!==1)throw new Error("auth_handoff_already_used");

  const out={userId:row.user_id};
  await env.DB.prepare(`DELETE FROM auth_login_handoffs WHERE id=?`).bind(row.id).run();
  return out;
}


function kakaoSuccessRedirect(env,redirectPath,payload){
  const origin=safeFrontendOrigin(env);
  const path=String(redirectPath||"/");
  const safePath=path==="/demo.html"?"/demo.html":"/";
  const target=new URL(safePath,origin);

  // 모바일 Safari/앱 전환 과정에서 fragment가 유실될 수 있어
  // 장기 세션 토큰 대신 1회용 handoff code를 query로 전달합니다.
  target.searchParams.set("guiin_auth_return","kakao");
  target.searchParams.set("handoff",payload.handoffCode);
  target.hash="loginComplete";
  return target.toString();
}

function kakaoErrorRedirect(env,redirectPath,error,stage="callback"){
  const origin=safeFrontendOrigin(env);
  const path=String(redirectPath||"/");
  const safePath=path==="/demo.html"?"/demo.html":"/";
  const target=new URL(safePath,origin);
  ...
  target.hash=new URLSearchParams({
    guiin_auth:"kakao",
    error:String(error||"login_failed").slice(0,120),
    stage:String(stage||"callback").slice(0,60)
  }).toString();
  return target.toString();
}
...
function productionReadiness(env){
  const missing=[],unsafe=[];
  const expectedRedirect="https://guiin-saju-api.blue-wls.workers.dev/api/auth/kakao/callback";
  const expectedOrigin="https://gwiinsaju.com";

  if(String(env.ENVIRONMENT||"")!=="production")missing.push("ENVIRONMENT");
  if(!dbReady(env))missing.push("DB");
  if(!env.OPENAI_API_KEY)missing.push("OPENAI_API_KEY");
  if(String(env.AI_CHAT_ENABLED||"")!=="true")missing.push("AI_CHAT_ENABLED");
  if(String(env.LOGIN_KAKAO_ENABLED||"")!=="true")missing.push("LOGIN_KAKAO_ENABLED");
  if(!String(env.KAKAO_REST_API_KEY||"").trim())missing.push("KAKAO_REST_API_KEY");
  if(!String(env.KAKAO_CLIENT_SECRET||"").trim())missing.push("KAKAO_CLIENT_SECRET");
  if(String(env.KAKAO_FRONTEND_ORIGIN||"").trim()!==expectedOrigin)missing.push("KAKAO_FRONTEND_ORIGIN");
  if(String(env.KAKAO_REDIRECT_URI||"").trim()!==expectedRedirect)missing.push("KAKAO_REDIRECT_URI");
  ...
  return{
    ...
    aiKeyPresent:!!env.OPENAI_API_KEY,
    kakaoConfigured:missing.every(x=>!x.startsWith("KAKAO_")&&x!=="LOGIN_KAKAO_ENABLED"),
    paymentsDisabled:String(env.NEW_PAYMENTS_ENABLED||"false")!=="true",
    walletAccountingDisabled:String(env.SERVER_WALLET_ENABLED||"false")!=="true",
    freeQuotaAccountingDisabled:String(env.SERVER_FREE_QUOTA_ENABLED||"false")!=="true",
    expectedFrontendOrigin:expectedOrigin,
    expectedKakaoRedirectUri:expectedRedirect,
    missing,unsafe
  };
}
...
function legalReadiness(env){
  const keys=[
    "TERMS_APPROVED","PRIVACY_APPROVED","REFUND_APPROVED","MERCHANT_CONTACT_READY",
    "ECOMMERCE_REPORTING_STATUS_CONFIRMED","PRIVACY_CONTACT_READY","PROCESSOR_LIST_CONFIRMED",
    "OVERSEAS_TRANSFER_DISCLOSURE_CONFIRMED","UNDER14_POLICY_CONFIRMED",
    "PG_DISCLOSURE_CONFIRMED","LEGAL_REVIEW_COMPLETED"
  ];
  const failed=keys.filter(k=>env[k]!=="true");
  return{ready:failed.length===0,failed};
}


function finalExternalGates(env){
  const keys=[
    "D1_STAGING_DEPLOYED","STAGING_E2E_PASSED","REAL_LOGIN_PROVIDER_VERIFIED",
    "SERVER_CHART_AUTHORITY_VERIFIED","PG_TEST_VERIFIED","INICIS_WEBHOOK_SIGNATURE_VERIFIED",
    "MERCHANT_CONTACT_READY","ECOMMERCE_REPORTING_STATUS_CONFIRMED","PRIVACY_CONTACT_READY",
    "PROCESSOR_LIST_CONFIRMED","OVERSEAS_TRANSFER_DISCLOSURE_CONFIRMED","UNDER14_POLICY_CONFIRMED",
    "LEGAL_REVIEW_COMPLETED","BACKUP_RESTORE_DRILL_PASSED"
  ];
  const failed=keys.filter(k=>env[k]!=="true");
  return{ready:failed.length===0,failed};
}
function finalCodeGate(env){
  // CI validates code-gates; production still requires explicit deploy confirmation.
  const ready=env.FINAL_CODE_GATE_VERIFIED==="true";
  return{ready,failed:ready?[]:["FINAL_CODE_GATE_VERIFIED"]};
}
function productionLaunchDecision(env){
  const code=finalCodeGate(env),external=finalExternalGates(env);
  const paymentFlag=env.NEW_PAYMENTS_ENABLED==="true";
  const allowed=code.ready&&external.ready&&paymentFlag;
  return{allowed,codeReady:code.ready,externalReady:external.ready,paymentsFlag:paymentFlag,
    codeFailed:code.failed,externalFailed:external.failed};
}

export default{
  async fetch(request,env){
    const reqId=requestId(),url=new URL(request.url),originHeader=request.headers.get("Origin")||"",allowed=pickOrigin(request);
    // OAuth 시작/콜백은 브라우저의 top-level navigation과 외부 Kakao redirect로 들어오므로
    // Origin 헤더에 의존해 차단하면 정상 로그인도 막힐 수 있습니다.
    // 정확히 이 두 경로만 Origin 검사 예외로 두고, 나머지 API는 기존 CORS 검사를 유지합니다.
    const oauthNavigationRoute=
      (request.method==="GET"&&url.pathname==="/api/auth/kakao/start")||
      (request.method==="GET"&&url.pathname==="/api/auth/kakao/callback");
    if(!oauthNavigationRoute&&originHeader&&!allowed)
    ...
    const productionInfra=productionReadiness(env);
    const productionProbe=
      request.method==="GET"&&(
        url.pathname==="/"||
        url.pathname==="/health"||
        url.pathname==="/api/production/readiness"
      );
    if(!productionInfra.ready&&!productionProbe){
      return json({
        ok:false,error:"production_infrastructure_not_ready",requestId:reqId,
        environment:productionInfra.environment,
        missing:productionInfra.missing,unsafe:productionInfra.unsafe
      },503,o);
    }
    ...
    if(request.method==="GET"&&(url.pathname==="/"||url.pathname==="/health")){
      const gate=paymentLaunchGate(env);
      const paymentsRuntimeEnabled=serviceStates.PAYMENTS_DISABLED==="false";
      const paymentsEnabled=env.NEW_PAYMENTS_ENABLED==="true"&&gate.ready&&paymentsRuntimeEnabled;
      return json({ok:true,service:"guiin-saju-api",version:WORKER_VERSION,answerSchemaVersion:ANSWER_SCHEMA_VERSION,environment:String(env.ENVIRONMENT||"unknown"),productionInfrastructureReady:productionInfra.ready,productionMissing:productionInfra.missing,productionUnsafe:productionInfra.unsafe,chatEnabled:env.AI_CHAT_ENABLED!=="false",databaseReady:dbReady(env),serverAccountingReady:false,paymentsEnabled,persistencePreview:true,accountingPreview:true,paymentFoundationPreview:true,paymentRecoveryPreview:true,securityHardeningPreview:true,securityRecoveryPreview:true,paymentLaunchReady:gate.ready,paymentLaunchFailed:gate.failed,runtimeMode:serviceStates.MAINTENANCE_MODE==="true"?"maintenance":serviceStates.READ_ONLY_MODE==="true"?"read_only":"normal",launchGatePreview:true},200,o);
    }
    ...
    if(request.method==="GET"&&url.pathname==="/api/auth/kakao/start"){
      if(!dbReady(env))return json({ok:false,error:"database_unavailable",requestId:reqId},503,o);
      if(!loginProviderEnabled(env,"kakao"))
        return json({ok:false,error:"provider_not_configured",provider:"kakao",requestId:reqId},503,o);
      try{
        const requestedRedirect=url.searchParams.get("redirectPath")||"/";
        const g=await guestSession(env,request);
        const login=await createLoginState(env,"kakao",g?.id||null,requestedRedirect);
        const authUrl=kakaoAuthorizeUrl(env,login.state);
        ...
        return json({ok:false,error:e?.message||"login_state_create_failed",requestId:reqId},500,o);
      }
    }
    ...
    if(request.method==="POST"&&url.pathname==="/api/auth/login/start"){
      ...
      const g=await guestSession(env,request);
      try{
        const requestedRedirect=b?.redirectPath??b?.redirect_path??"/";
        const login=await createLoginState(env,provider,g?.id||null,requestedRedirect);
        const authUrl=provider==="kakao"?kakaoAuthorizeUrl(env,login.state):null;
        return json({
          ok:true,
          requestId:reqId,
          provider,
    ...
    if(request.method==="GET"&&url.pathname==="/api/auth/kakao/callback"){
      if(!loginProviderEnabled(env,"kakao"))
        return json({ok:false,error:"provider_not_configured",provider:"kakao",requestId:reqId},503,o);
      ...
      let loginState=null;
      let loginStage="state_validation";

      try{
        if(!state)throw new Error("login_state_required");
        loginState=await consumeLoginState(env,"kakao",state);

        loginStage="authorization_code";
        if(oauthError)throw new Error(oauthError==="access_denied"?"kakao_access_denied":"kakao_authorization_failed");
        if(!authCode)throw new Error("kakao_authorization_code_missing");

        loginStage="token_exchange";
        const token=await exchangeKakaoCode(env,authCode);

        loginStage="userinfo";
        const kakao=await fetchKakaoUser(token.access_token);

        loginStage="user_upsert";
        const user=await getOrCreateKakaoUser(env,kakao);

        loginStage="handoff_issue";
        const handoff=await createAuthHandoff(env,user.userId);

        loginStage="redirect";

        await recordSecurityEvent(env,{
          eventType:"login_success",severity:"warning",requestId:reqId,route:url.pathname,request,
          subject:{type:"user",id:user.userId},metadata:{provider:"kakao",newUser:user.newUser}
        });

        return new Response(null,{status:302,headers:{
          "Location":kakaoSuccessRedirect(env,loginState.redirect_path,{
            handoffCode:handoff.code,userId:user.userId,newUser:user.newUser
          }),
          "Cache-Control":"no-store",
          ...securityHeaders()
        }});
      }catch(e){
        const code=String(e?.message||"kakao_login_failed");
        await recordSecurityEvent(env,{
          eventType:"login_failed",severity:"warning",requestId:reqId,route:url.pathname,request,
          metadata:{provider:"kakao",error:code,stage:loginStage}
        });
        return new Response(null,{status:302,headers:{
          "Location":kakaoErrorRedirect(env,loginState?.redirect_path||"/",code,loginStage),
          "Cache-Control":"no-store",
          ...securityHeaders()
        }});
      }
    ...
    if(request.method==="POST"&&url.pathname==="/api/auth/kakao/handoff"){
      if(!dbReady(env))return json({ok:false,error:"database_unavailable",requestId:reqId},503,o);
      let b;try{b=await request.json()}catch(_){return json({ok:false,error:"invalid_json",requestId:reqId},400,o)}
      try{
        const h=await consumeAuthHandoff(env,b?.code);
        const session=await createUserSession(env,h.userId,request);
        await recordSecurityEvent(env,{
          eventType:"login_handoff_consumed",severity:"warning",requestId:reqId,route:url.pathname,request,
          subject:{type:"user",id:h.userId},metadata:{provider:"kakao"}
        });
        return json({
          ok:true,requestId:reqId,provider:"kakao",
          token:session.sessionToken,userId:h.userId
        },200,o);
      }catch(e){
        const code=String(e?.message||"auth_handoff_failed");
        const status=/expired|not_found|already_used|invalid/.test(code)?400:500;
    ...
    if(request.method==="GET"&&url.pathname==="/api/production/readiness"){
      return json({
        ok:true,requestId:reqId,version:WORKER_VERSION,...productionReadiness(env)
      },200,o);
    }

    if(request.method==="GET"&&url.pathname==="/api/final-launch/readiness"){
      const x=productionLaunchDecision(env);
      return json({ok:true,requestId:reqId,version:WORKER_VERSION,
        codeReady:x.codeReady,externalReady:x.externalReady,
        paymentsFlag:x.paymentsFlag,launchReady:x.allowed,
        codeFailed:x.codeFailed,externalFailed:x.externalFailed},200,o);
    }

    if(request.method==="GET"&&url.pathname==="/api/legal/readiness"){
      const legal=legalReadiness(env);
      return json({ok:true,requestId:reqId,version:WORKER_VERSION,legalReady:legal.ready,failed:legal.failed},200,o);
    }
    ...
    if(request.method==="GET"&&url.pathname==="/api/me"){
      if(s.type!=="user")return json({ok:false,authenticated:false,error:"user_session_required",requestId:reqId},401,o);
      const user=await env.DB.prepare(`SELECT id,status,display_name,created_at,updated_at FROM users WHERE id=? LIMIT 1`).bind(s.id).first();
      if(!user||user.status!=="active")
        return json({ok:false,authenticated:false,error:"user_not_active",requestId:reqId},401,o);
      const identity=await env.DB.prepare(`SELECT provider,email_normalized,email_verified FROM auth_identities WHERE user_id=? ORDER BY created_at ASC LIMIT 1`).bind(s.id).first();
      return json({
        ok:true,
        authenticated:true,
        requestId:reqId,
        user:{
          ...
          provider:identity?.provider||"kakao",
          email:identity?.email_verified?identity?.email_normalized:null
        }
      },200,o);
    }
    ...
    if(request.method==="POST"&&url.pathname==="/api/orders"){
      const launch=paymentLaunchGate(env);
      if(env.NEW_PAYMENTS_ENABLED!=="true"||!launch.ready||serviceStates.PAYMENTS_DISABLED!=="false"){
        return json({ok:false,error:"payment_launch_not_ready",requestId:reqId},503,o);
      }
    ...
    if(request.method==="GET" && url.pathname.startsWith(orderPrefix)){
      const orderId=url.pathname.slice(orderPrefix.length);

      if(orderId && orderId.indexOf("/")===-1){
        const order=await ownedOrder(env,s,orderId);
